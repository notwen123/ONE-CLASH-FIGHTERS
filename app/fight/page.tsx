    ...
    cpuMovementRef.current = window.setInterval(() => {
      ...
      if (isPlayerNearby && !cpuAttackCooldown && cpuState === "idle") {
        ...
        if (attackChance < 0.75) {
          // 75% chance to attack when close (increased aggression)
          ...
          if (attackChance < 0.3) {
            ...
            if (
              ...
              if (playerState === "defence") {
                ...
                setPlayerHealth((prev) => Math.max(0, prev - Math.round(8 * damageMultiplier))) // Increased punch damage
                setIsPlayerHit(true)
              ...
              if (playerHealth - (playerState === "defence" ? 1 : Math.round(8 * difficulty)) <= 0) {
                endGame("cpu")
              }
            }
            ...
            if (
              ...
              if (playerState === "defence") {
                ...
                // Apply difficulty multiplier to damage
                const damageMultiplier = difficulty
                setPlayerHealth((prev) => Math.max(0, prev - Math.round(12 * damageMultiplier))) // Increased kick damage
                setIsPlayerHit(true)
                setTimeout(() => setIsPlayerHit(false), 300)
              }
              ...
              if (playerHealth - (playerState === "defence" ? 1 : Math.round(12 * difficulty)) <= 0) {
                endGame("cpu")
              }
            }
      ...
      if (cpuIdleTime > 2) {
        // Reduced from 3 to 2 to make CPU move more aggressively
        // Move towards player
        setCpuPosition((prev) => {
          ...
          // Calculate new position - increased speed from 15 to 20
          const newPosition = Math.max(50, Math.min(window.innerWidth - 150, prev + direction * 20))
      ...
      // Random movement chance (higher probability - increased from 0.3 to 0.7)
      if (Math.random() < 0.7) {
        // Move towards player
        setCpuPosition((prev) => {
          const direction = cpuCenterX > playerCenterX ? 1 : -1
